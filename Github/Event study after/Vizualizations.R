...
combined_data <- read_delim("combineddata_event.csv", delim = ";", locale = locale(decimal_mark = ",")) %>%
  mutate(filing_date = as.Date(filing_date, format = "%d-%m-%Y"))

# Merge metadata into CAR_multiple_windows
CAR_full <- CAR_multiple_windows %>%
  left_join(combined_data, by = c("ticker" = "ticker", "Event_Date" = "filing_date"))
...
CAR_full$breach_type <- reorder(CAR_full$breach_type, CAR_full$CAR_2_post, FUN = median)
CAR_full$breach_type2 <- reorder(CAR_full$breach_type2, CAR_full$CAR_2_post, FUN = median)
CAR_full$overall_sector <- reorder(CAR_full$overall_sector, CAR_full$CAR_2_post, FUN = median)

# --- Step 2: Visualizations ---

# Boxplots
ggplot(CAR_full, aes(x = breach_type2, y = CAR_2_post)) +
  geom_boxplot(fill = "steelblue", color = "black") +
  labs(title = "CAR (+2) Across Breach Type 2", x = "Breach Type 2", y = "CAR (+2)") +
  theme_apa()

ggplot(CAR_full, aes(x = overall_sector, y = CAR_2_post)) +
  geom_boxplot(fill = "coral", color = "black") +
  labs(title = "CAR (+2) Across Overall Sectors", x = "Overall Sector", y = "CAR (+2)") +
  theme_apa()

ggplot(CAR_full, aes(x = breach_size_group, y = CAR_2_post)) +
  geom_boxplot(fill = "grey", color = "black") +
  labs(title = "CAR (+2) Across Overall Sectors", x = "Overall Sector", y = "CAR (+2)") +
  theme_apa()

# Kruskal-Wallis Tests
kruskal.test(CAR_2_post ~ breach_type2, data = CAR_full)
kruskal.test(CAR_2_post ~ overall_sector, data = CAR_full)

# Boxplots and Wilcoxon for breach size and market cap
ggplot(CAR_full, aes(x = breach_size_group, y = CAR_2_post)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  labs(title = "CAR (+2) by Breach Size", x = "Breach Size", y = "CAR (+2)") +
  theme_apa()

wilcox.test(CAR_2_post ~ breach_size_group, data = CAR_full)

ggplot(CAR_full, aes(x = market_cap_group, y = CAR_2_post)) +
  geom_boxplot(fill = "lightgreen", color = "black") +
  labs(title = "CAR (-4,+3) by Market Cap", x = "Market Cap Group", y = "CAR (-4 to +3)") +
  theme_minimal(base_size = 14) + theme(plot.title = element_text(face = "bold", hjust = 0.5))

wilcox.test(CAR_2_post ~ market_cap_group, data = CAR_full)
...
variables_to_summarize <- list(
  ...
  "CAR_10_10" = CAR_multiple_windows$CAR_10_10_post,
  "CAR_6_6_post" = CAR_multiple_windows$CAR_6_6_post,
  "CAR_2_6_post" = CAR_multiple_windows$CAR_2_6_post,
  "CAR_2_post" = CAR_multiple_windows$CAR_2_post
)
...
write_csv2(CAR_full, "carfullpost.csv")
