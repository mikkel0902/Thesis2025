...
event_window66  <- calc_event_window(event_data, -6, 6)
...
CAR_multiple_windows <- event_window10 %>%
  group_by(ticker, Event_Date) %>%
  summarise(
    CAR_10_10_post = sum(AR[Relative_Day >= -10 & Relative_Day <= 10], na.rm = TRUE),
    CAR_6_6_post = sum(AR[Relative_Day >= -6 & Relative_Day <= 6], na.rm = TRUE),
    CAR_2_6_post = sum(AR[Relative_Day >= 2 & Relative_Day <= 6], na.rm = TRUE),
    CAR_2_post = sum(AR[Relative_Day >= 2 & Relative_Day <= 2], na.rm = TRUE),
  )
...
CARs4  <- calc_CAR_series(event_window66)
...
average_CARs <- CAR_multiple_windows %>%
  summarise(
    ...
    Avg_CAR_6_6_post = mean(CAR_6_6_post, na.rm = TRUE),
    Avg_CAR_2_6_post = mean(CAR_2_6_post, na.rm = TRUE),
    Avg_CAR_2_post = mean(CAR_2_post, na.rm = TRUE)
  )
...
ggplot(CAR_multiple_windows, aes(x = CAR_10_10_post)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "grey100", color = "black") +
  geom_density(color = "black", size = 1) +
...
t_test_2_6_post <- t.test(CAR_multiple_windows$CAR_2_6_post)
t_test_2_post <- t.test(CAR_multiple_windows$CAR_2_post)
...
car_tests <- tibble(
  Window = c("-10 to +10", "-6 to +6", "+2 to +6", "+2"),
  Mean_CAR = c(
    mean(CAR_multiple_windows$CAR_10_10_post, na.rm = TRUE),
    mean(CAR_multiple_windows$CAR_6_6_post, na.rm = TRUE),
    mean(CAR_multiple_windows$CAR_2_6_post, na.rm = TRUE),
    mean(CAR_multiple_windows$CAR_2_post, na.rm = TRUE)
  ),
  t_stat = c(
    t_test_10_10_post$statistic,
    t_test_6_6_post$statistic,
    t_test_2_6_post$statistic,
    t_test_2_post$statistic
  ),
  p_value = c(
    t_test_10_10_post$p.value,
    t_test_6_6_post$p.value,
    t_test_2_6_post$p.value,
    t_test_2_post$p.value
  )
) %>%
...
# Wilcoxon tests
wilcox_test_car_4 <- wilcox.test(CAR_multiple_windows$CAR_2_post, mu = 0)
